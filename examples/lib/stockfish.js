import { INITIAL_FEN } from 'next-chessground';

const STOCKFISH_PATH = process.env.STOCKFISH_PATH || '/stockfish.asm.js';

/**
 * Minimal UCI client for the Stockfish build in `public/stockfish.asm.js`.
 * The engine runs in a Web Worker shared by the whole page: `init` once, then
 * `set_position(fen)` and `go_time(ms)` for every move, and `quit` when done.
 * The engine is not part of next-chessground 2.0 - this is demo code.
 */
class Stockfish {
  constructor(path) {
    ...
    if (!window.chessEngineWorker) {
      window.chessEngineWorker = new Worker(path ?? STOCKFISH_PATH);
    }
    this.skillLevel = 20;
    this.maxDepth = 40;
  ...
  /**
   * Idempotent: the worker is shared and dropped here, so the next page that
   * needs an engine starts a fresh one instead of talking to a dead process.
   */
  quit() {
    if (typeof window === 'undefined' || !window.chessEngineWorker) {
      return;
    }
